...
def volley_read_annotations(path):
    ...
    gact_to_id = {name: i for i, name in enumerate(ACTIVITIES)}  # a dictionary to map group activity to id
    ...
    with open(path) as f:
        for l in f.readlines():
            values = l[:-1].split(' ')
            file_name = values[0]
            activity = gact_to_id[values[1]]  # group activity
            ...
            bboxes = np.array([_read_bbox(values[i:i+4])
                               for i in range(0, 5*num_people, 5)])  # bb for each individual
